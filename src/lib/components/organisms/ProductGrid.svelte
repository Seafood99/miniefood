<script lang="ts">
		...
		products = [
			{
				...
				description:
					'Rendang autentik dengan bumbu rempah pilihan, daging sapi premium yang empuk dan lezat.',
				price: 'Rp 85.000',
			...
			},
			{
				...
				description:
					'Gudeg tradisional Yogyakarta dengan cita rasa manis yang khas dan aroma santan yang menggoda.',
				price: 'Rp 65.000',
	...
	const gridClasses = ['relative py-20 sm:py-24 lg:py-32 overflow-hidden', className]
		.filter(Boolean)
		.join(' ');
...
<section class={gridClasses} {...restProps}>
	...
	<div
		...
		style="background: linear-gradient(180deg, #2B1B17 0%, #3d2b26 50%, #2B1B17 100%);"
	></div>

	<!-- Animated Overlay -->
	<div class="absolute inset-0 opacity-15">
		<div
			class="animate-gradient absolute inset-0 bg-linear-to-br from-[#FFD600]/20 via-transparent to-[#E53935]/20"
		></div>
	</div>
	...
	<!-- Floating Elements -->
	<div
		class="animate-float absolute top-10 left-[10%] h-40 w-40 rounded-full opacity-10 blur-3xl"
		style="background: #FFD600;"
	></div>
	<div
		class="animate-float absolute right-[10%] bottom-20 h-56 w-56 rounded-full opacity-10 blur-3xl"
		style="background: #E53935; animation-delay: 2s;"
	></div>
	<div
		class="animate-float absolute top-1/3 right-[20%] h-32 w-32 rounded-full opacity-5 blur-3xl"
		style="background: #F2E2C4; animation-delay: 3s;"
	></div>
	...
	<div class="relative z-10 container mx-auto px-4 sm:px-6 lg:px-8">
		...
		<div class="mx-auto mb-16 max-w-3xl text-center">
			<!-- Decorative Line -->
			<div class="animate-fade-in mb-6 flex items-center justify-center gap-4">
				<div
					class="h-px w-16 sm:w-24"
					style="background: linear-gradient(to right, transparent, #FFD600);"
				></div>
				<div class="h-2 w-2 rounded-full" style="background: #FFD600;"></div>
				<div
					class="h-px w-16 sm:w-24"
					style="background: linear-gradient(to left, transparent, #FFD600);"
				></div>
			</div>
			...
			<Typography
				tag="h2"
				variant="h2"
				class="animate-slide-up mb-4 leading-tight font-black tracking-tight"
			...
			{#if subtitle}
				<Typography
					tag="h3"
					variant="h4"
					class="animate-slide-up mb-6 text-xl font-semibold sm:text-2xl"
			...
			{#if description}
				<Typography
					variant="lead"
					class="animate-fade-in text-base leading-relaxed sm:text-lg"
			...
			<div
				class="animate-fade-in mt-8 flex items-center justify-center gap-4"
				style="animation-delay: 0.3s;"
			>
				<div
					class="h-px w-20 sm:w-32"
					style="background: linear-gradient(to right, transparent, rgba(242, 226, 196, 0.3), transparent);"
				></div>
			</div>
		</div>
		...
		<div class={productGridClasses}>
			{#each products as product, index (product.id)}
				<div class="animate-slide-up" style="animation-delay: {0.1 * index}s;">
					<Card
						title={product.title}
		...
		{#if showViewAll}
			<div class="animate-fade-in mt-16 text-center" style="animation-delay: 0.5s;">
				<a
					href={viewAllHref}
					class="group inline-flex min-w-[250px] items-center justify-center gap-3 rounded-xl px-10 py-5 text-base font-bold transition-all duration-300 hover:scale-105 hover:shadow-2xl sm:text-lg"
